...
LangSys::LangSys()
{
    ...
    // Status
    m_data["status.python_ok"] = {"Python", "Python"};
    m_data["status.python_no"] = {"Python", "Python"};
    m_data["status.project"] = {"项目:", "Project:"};

    // Git
    m_data["git.title"] = {"GitHub 下载", "GitHub download"};
    ...
    // Env
    m_data["env.python"] = {"Python 状态", "Python"};
    m_data["env.ready"] = {"Python 就绪", "Python ready"};
    m_data["env.recheck"] = {"重新检测", "recheck"};
    m_data["env.version"] = {"版本", "version"};
    m_data["env.nofound"] = {"Python 未找到", "Python not found"};
    ...
    m_data["srv.on"] = {"运行中", "running"};
    m_data["srv.off"] = {"未运行", "not running"};
